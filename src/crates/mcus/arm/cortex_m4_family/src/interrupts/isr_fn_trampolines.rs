use core::convert::{AsMut, From};

use smeg_kernel::docs;
use smeg_kernel::interrupts::IsrContext;

pub trait IsrTrampolineContext: IsrContext + From<super::IsrContext> + AsMut<super::IsrContext> { }
impl<T: IsrContext + From<super::IsrContext> + AsMut<super::IsrContext>> IsrTrampolineContext for T { }
    ...
    ( @__(
        ...
        unsafe extern "C" fn $trampoline_fn_name<D>() -> !
            ...
            unsafe extern "C" fn trampoline<D>(stack_frame: *mut ::smeg_mcu_arm_cortex_m4_family::interrupts::IsrBasicStackFrame)
                ...
                let mut context = <D::IsrContext>::from(::smeg_mcu_arm_cortex_m4_family::interrupts::IsrContext::from(stack_frame));
                unsafe {
