...
use super::register_definition::RegisterDefinition;
...
pub fn arm_register(_args: TokenStream, items: TokenStream) -> TokenStream {
    ...
    match register_type_name_from(&derive.data) {
        Some(("i8", type_ident)) => RegisterDefinition::<i8>::generate(&derive, &type_ident),
        Some(("u8", type_ident)) => RegisterDefinition::<u8>::generate(&derive, &type_ident),

        Some(("i16", type_ident)) => RegisterDefinition::<i16>::generate(&derive, &type_ident),
        Some(("u16", type_ident)) => RegisterDefinition::<u16>::generate(&derive, &type_ident),

        Some(("i32", type_ident)) => RegisterDefinition::<i32>::generate(&derive, &type_ident),
        Some(("u32", type_ident)) => RegisterDefinition::<u32>::generate(&derive, &type_ident),

        Some(("i64", type_ident)) => RegisterDefinition::<i64>::generate(&derive, &type_ident),
        Some(("u64", type_ident)) => RegisterDefinition::<u64>::generate(&derive, &type_ident),

        Some(("isize", type_ident)) => RegisterDefinition::<isize>::generate(&derive, &type_ident),
        Some(("usize", type_ident)) => RegisterDefinition::<usize>::generate(&derive, &type_ident),
