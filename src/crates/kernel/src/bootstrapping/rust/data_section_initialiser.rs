...
#[doc = docs::side_by_side_md!("DataSectionInitialiser")]
pub unsafe trait DataSectionInitialiser {
    #[doc = docs::side_by_side_md!("DataSectionInitialiser.load_data_section")]
...
mod tests {
    ...
    use crate::bootstrapping::rust::DataSectionInitialiser;
    ...
    fn load_data_section__called_with_ram_start_equal_to_ram_past_end__expect_ram_is_not_modified<T: DataSectionInitialiser>(initialiser: T) {
        ...
        unsafe {
            ...
            initialiser.load_data_section(&mut *ram_start, ram_past_end, &rom[0]);
        }

        expect!(unsafe { ram[0].assume_init() }).to_equal(original_value);
    }

    fn load_data_section__called__expect_ram_is_loaded_up_to_past_end_with_given_rom_contents<T: DataSectionInitialiser>(initialiser: T) {
        ...
        let all_excluding_past_end = unsafe {
            initialiser.load_data_section(&mut head[0], tail.last().unwrap(), &rom[0]);
            ram.pop();
    ...
    fn load_data_section__called__expect_ram_past_end_is_not_modified<T: DataSectionInitialiser>(initialiser: T) {
        let mut ram = any_vec_filled_using(2..1024, || MaybeUninit::new(any_usize()));
