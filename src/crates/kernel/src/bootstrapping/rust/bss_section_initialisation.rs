...
#[doc = docs::side_by_side_md!("BssSectionInitialisation")]
pub unsafe trait BssSectionInitialisation {
    #[doc = docs::side_by_side_md!("BssSectionInitialisation.fill_bss_section")]
    unsafe fn fill_bss_section(start: &mut MaybeUninit<usize>, past_end: &MaybeUninit<usize>, fill_value: u8);
}

#[doc = docs::side_by_side_md!("BssSectionInitialiserWithChecks")]
pub struct BssSectionInitialiserWithChecks;

unsafe impl BssSectionInitialisation for BssSectionInitialiserWithChecks {
...
unsafe impl BssSectionInitialisation for BssSectionInitialiserWithoutChecks {
...
mod tests {
    ...
    fn fill_bss_section__called_with_start_equal_to_past_end__expect_block_is_not_filled<I: BssSectionInitialisation>() {
        ...
        unsafe {
            ...
            I::fill_bss_section(&mut *start, past_end, fill_value);
        }

        expect!(unsafe { block[0].assume_init() }).to_equal(original_value);
    }

    fn fill_bss_section__called__expect_block_is_filled_up_to_past_end_with_given_byte<I: BssSectionInitialisation>() {
        ...
        let all_excluding_past_end = unsafe {
            I::fill_bss_section(&mut head[0], tail.last().unwrap(), fill_value);
            block.pop();
    ...
    fn fill_bss_section__called__expect_block_past_end_is_not_filled<I: BssSectionInitialisation>() {
        let mut block = any_vec_filled_using(2..1024, || MaybeUninit::new(any_usize()));
